    ...
    if (isObj) {
      ...
      if (defaultString) {
        ...
        m_connectionIDBytes = CacheableBytes::create(
            std::vector<int8_t>(input.currentBufferPosition(), input.currentBufferPosition() +
            lenObj));
        input.advanceCursor(lenObj);
      }
    }
...
void TcrMessage::readUniqueIDObjectPart(DataInput& input) {
  ...
  if (lenObj > 0) {
    m_value = CacheableBytes::create(
        std::vector<int8_t>(input.currentBufferPosition(), input.currentBufferPosition() + lenObj));
    input.advanceCursor(lenObj);
  }
}
...
void TcrMessage::createUserCredentialMessage(TcrConnection* conn) {
  ...
  auto credBytes =
      CacheableBytes::create(std::vector<int8_t>(dOut->getBuffer(), dOut->getBuffer() +
