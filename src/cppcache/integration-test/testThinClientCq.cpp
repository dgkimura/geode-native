...
DUNIT_TASK_DEFINITION(CLIENT1, StepThree)
  {
    ...
    PoolPtr pool = CacheHelper::getHelper().getCache()->getPoolManager().find(regionNamesCq[0]);
    QueryServicePtr qs;
    if (pool != nullptr) {
...
DUNIT_TASK_DEFINITION(CLIENT1, createCQ)
  {
    ...
    PoolPtr pool = CacheHelper::getHelper().getCache()->getPoolManager().find(regionName);
...
DUNIT_TASK_DEFINITION(CLIENT1, createCQ_Pool)
  {
    PoolPtr pool = CacheHelper::getHelper().getCache()->getPoolManager().find("__TEST_POOL1__");
    QueryServicePtr qs = pool->getQueryService();
    CqAttributesFactory cqFac;
    auto cqLstner = std::make_shared<MyCqStatusListener>(100);
    cqFac.addCqListener(cqLstner);
    ...
    PoolPtr pool2 = CacheHelper::getHelper().getCache()->getPoolManager().find("__TEST_POOL2__");
...
void executeCq(const char* poolName, const char* name) {
  PoolPtr pool = CacheHelper::getHelper().getCache()->getPoolManager().find(poolName);
  QueryServicePtr qs;
  if (pool != nullptr) {
    qs = pool->getQueryService();
  }
