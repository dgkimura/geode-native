...
int32_t Security::initValues(int32_t numKeys, int32_t siz, bool useDefault) {
  ...
  std::vector<int8_t> buf(siz);
  memset(&buf[0], 'V', siz);
  int32_t rsiz = (siz <= 20) ? siz : 20;
  for (int32_t i = 0; i < m_MaxValues; i++) {
    GsRandom::getAlphanumericString(rsiz, reinterpret_cast<char *>(buf.data()));
    m_CValue[i] =
        CacheableBytes::create(buf);
  }
  return siz;
}
...
int32_t Security::doEntryOperations() {
  ...
  std::vector<int8_t> valBuf(valSize + 1);
  memset(&valBuf[0], 'A', valSize);
  valBuf[valSize] = 0;
